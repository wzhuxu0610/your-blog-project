...
export async function onRequest(context) {
  ...
  async function handleUploadLogo(request, env) {
    ...
    if (!verifyToken(token)) return new Response(JSON.stringify({ success: false, message: "请先登录" }), { status: 401 });
    try {
      const formData = await request.formData();
      const file = formData.get("logo");
      if (!file) return new Response(JSON.stringify({ success: false, message: "请选择图片" }), { status: 400 });
      if (file.size > 2 * 1024 * 1024) return new Response(JSON.stringify({ success: false, message: "不能超过2MB" }), { status: 400 });
      if (!file.type.startsWith("image/")) return new Response(JSON.stringify({ success: false, message: "请上传图片" }), { status: 400 });

      const buf = await file.arrayBuffer();
      const key = "logo_" + Date.now() + "." + (file.type.split("/")[1] || "png");
      ...
      return new Response(JSON.stringify({ success: false, message: "上传失败" }), { status: 500 });
    }
  }
  ...
  async function handleUploadImage(request, env) {
    ...
    try {
      ...
      if (file.size > 5 * 1024 * 1024) return new Response(JSON.stringify({ success: false }), { status: 400 });
      const buf = await file.arrayBuffer();
      const key = "img_" + Date.now() + "_" + Math.random().toString(36).slice(2,8);
      await env.BLOG_KV.put(key, buf, { metadata: { type: file.type } });
      const host = request.headers.get('host');
      const protocol = request.url.startsWith('https') ? 'https' : 'http';
      return new Response(JSON.stringify({ success: true, url: `${protocol}://${host}/api/i/${key}` }), {
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
      });
    } catch {
  ...
  async function handleGetButtons(env) {
    try {
      ...
      return new Response(data || JSON.stringify(DEFAULT_BUTTONS), {
        headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
      });
    } catch {
  ...
  async function handleGetFeaturedPost(env) {
    try {
      ...
      const res = tops.length > 0 ? tops[0] : (all.length > 0 ? all[0] : { isEmpty: true });
  ...
  async function handleGetBlog(id, env) {
    ...
    if (!val) return new Response(JSON.stringify({ error: "不存在" }), { status: 404 });
    const p = JSON.parse(val);
