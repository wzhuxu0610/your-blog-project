// /functions/[[route]].js - Cloudflare Pages Functions（完整版）
...
async function loadLogo(){
  ...
  }catch(e){console.error(e);currentLogoUrl="";updateLogoDisplay(null);}
}

function updateLogoDisplay(url){
  var img=document.getElementById("sidebarLogo"),ph=document.getElementById("sidebarLogoPlaceholder");
  if(!img||!ph) return;
  if(url&&url!==""){
    img.src=url+"?v="+logoVersion;
...
function renderQuickButtons(){
  ...
  for(var i=0;i<quickButtons.length;i++){
    ...
    if(btn.enabled!==false){
      html+='<a href="'+escapeHtml(btn.url)+'" class="quick-btn" target="_blank">'+escapeHtml(btn.name)+'</a>';
    }
  }
...
function renderArticlesList(){
  ...
  for(var i=0;i<allPosts.length;i++){
    var p=allPosts[i];
    var active=(currentPostId===p.id)?"active":"";
    html+='<div class="article-item '+active+'" onclick="loadPost(\''+p.id+'\')"><div class="article-title">'+escapeHtml(p.title)+'</div><div class="article-time">'+new Date(p.time).toLocaleDateString()+'</div></div>';
  }
  container.innerHTML=html;
}
...
function displayPost(post){
  ...
  var editHtml="";
  if(token){
    editHtml='<div style="margin-top:24px;display:flex;gap:12px"><button class="btn-secondary" onclick="editPost(\''+post.id+'\')">编辑文章</button><button class="btn-danger" onclick="deletePost(\''+post.id+'\')">删除文章</button></div>';
  }
  var html='<div class="content-card"><h1 class="post-title">'+escapeHtml(post.title)+'</h1><div class="post-meta">发布时间：'+new Date(post.time).toLocaleString()+'</div>'+(post.img?'<img src="'+escapeHtml(post.img)+'" class="post-img" alt="封面">':'')+'<div class="post-content">'+(post.content?post.content.replace(/\\n/g,"<br>"):"")+'</div>'+editHtml+'</div>';
  container.innerHTML=html;
}
...
function updateNav(){
  ...
  var pb=document.getElementById("publishNavBtn"),mb=document.getElementById("manageNavBtn"),lb=document.getElementById("loginNavBtn"),lob=document.getElementById("logoutNavBtn"),eb=document.getElementById("editButtonsBtn");
  if(pb) pb.classList.toggle("hidden",!loggedIn);
  if(mb) mb.classList.toggle("hidden",!loggedIn);
  if(lb) lb.classList.toggle("hidden",loggedIn);
  if(lob) lob.classList.toggle("hidden",!loggedIn);
  if(eb) eb.classList.toggle("hidden",!loggedIn);
}
...
async function uploadImg(){
  ...
  fd.append("file",file);
  try{
    var res=await fetch("/api/upload",{method:"POST",headers:{"Authorization":"Bearer "+localStorage.getItem("token")},body:fd});
    var data=await res.json();
    if(data.success){
      currentImage=data.url;
      document.getElementById("preview").innerHTML='<img src="'+data.url+'" style="max-width:150px;margin:10px 0"><br><button onclick="removeImg()">移除图片</button>';
      alert("上传成功");
    }else alert("上传失败");
  }catch(e){alert("上传失败");}
}
...
function showManage(){
  var token=localStorage.getItem("token");
  if(!token){showLogin();return;}
  var logoHtml=currentLogoUrl?'<img src="'+currentLogoUrl+'?v='+logoVersion+'" style="width:60px;height:60px;border-radius:50%">':'<div style="width:60px;height:60px;border-radius:50%;background:#f1f3f5;display:flex;align-items:center;justify-content:center;font-size:30px">📷</div>';
  document.getElementById("mainContent").innerHTML='<div class="content-card"><h2>管理后台</h2><div style="margin-bottom:20px;padding:16px;background:#f8f9fa;border-radius:8px"><h3>Logo设置</h3><div style="margin:12px 0">'+logoHtml+'</div><button onclick="document.getElementById(\'logoUpload\').click()">更换Logo</button>'+(currentLogoUrl?'<button class="btn-danger" style="margin-left:10px" onclick="deleteLogo()">恢复默认</button>':'')+'<input type=file id=logoUpload accept="image/*" style="display:none" onchange="uploadLogoFile(this.files[0])"></div><div><h3>快捷按钮管理</h3><button onclick="openButtonsModal()">管理10个快捷按钮</button></div><div style="margin-top:20px"><h3>文章列表</h3><div id=managePosts></div></div></div>';
  renderManagePosts();
}

async function renderManagePosts(){
  var c=document.getElementById("managePosts");
  if(!c)return;
  if(allPosts.length===0){c.innerHTML="<p>暂无文章</p>";return;}
  var html="";
  for(var i=0;i<allPosts.length;i++){
    var p=allPosts[i];
    html+='<div style="border:1px solid #e9ecef;border-radius:6px;padding:10px;margin-bottom:8px;display:flex;justify-content:space-between"><div><strong>'+escapeHtml(p.title)+'</strong><br><small>'+new Date(p.time).toLocaleDateString()+'</small></div><div><button class="btn-secondary" style="margin-right:6px" onclick="editPost(\''+p.id+'\')">编辑</button><button class="btn-danger" onclick="deletePost(\''+p.id+'\')">删除</button></div></div>';
  }
  c.innerHTML=html;
}
...
async function uploadLogoFile(f){
  ...
  fd.append("logo",f);
  try{
    var res=await fetch("/api/logo/upload",{method:"POST",headers:{"Authorization":"Bearer "+localStorage.getItem("token")},body:fd});
    var data=await res.json();
...
function openButtonsModal(){
  ...
  for(var i=0;i<quickButtons.length;i++){
    var btn=quickButtons[i];
    html+='<div class="btn-item"><input type="text" placeholder="按钮名称" value="'+escapeHtml(btn.name)+'" id="btn_name_'+i+'"><input type="text" placeholder="链接地址" value="'+escapeHtml(btn.url)+'" id="btn_url_'+i+'"><label><input type="checkbox" id="btn_enabled_'+i+'"'+(btn.enabled!==false?" checked":"")+'> 启用</label></div>';
  }
  container.innerHTML=html;
...
async function saveButtons(){
  var nb=[];
  for(var i=0;i<quickButtons.length;i++){
    var ni=document.getElementById("btn_name_"+i),ui=document.getElementById("btn_url_"+i),ei=document.getElementById("btn_enabled_"+i);
    nb.push({name:ni?ni.value:"按钮"+(i+1),url:ui?ui.value:"https://example.com",enabled:ei?ei.checked:true});
  }
  try{
    var res=await fetch("/api/buttons",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+localStorage.getItem("token")},body:JSON.stringify(nb)});
...
function insertLink(){
  ...
  var s=ta.selectionStart,e=ta.selectionEnd,sel=ta.value.substring(s,e),url=prompt("请输入链接地址:","https://");
  if(url&&url!=="https://"){
    var text=sel||prompt("请输入链接文字:",url);
    if(text){
...
function formatText(tag){
  var ta=document.getElementById("contentText");
  if(!ta)return;
  var s=ta.selectionStart,e=ta.selectionEnd,sel=ta.value.substring(s,e),formatted="";
  if(tag==="bold") formatted="<strong>"+sel+"</strong>";
  else if(tag==="italic") formatted="<em>"+sel+"</em>";
  else if(tag==="underline") formatted="<u>"+sel+"</u>";
  else if(tag==="h3") formatted="<h3>"+sel+"</h3>";
  if(formatted){
    ta.value=ta.value.substring(0,s)+formatted+ta.value.substring(e);
    ta.focus();
  }
}
